...
try:
    ...
    logger = _logging.getLogger("funnels_create_with_uploads")
...
class CreateFunnelWithUploadsCommand(BaseCommand):
    """
    Creates a Funnel with optional uploads:
    ...
    async def execute(
        ...
        t0 = time.monotonic()
        logger.info("[funnels] start name=%s user_id=%s", getattr(payload, "name", None), user_id)
        gcs = get_gcs()
