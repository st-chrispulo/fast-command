# commands/components/layout/update.py
import os
import re
import json
import mimetypes
...
class UpdateLayoutWithUploadsCommand(BaseCommand):
    ...
    NOTE: stores GCS *keys* (not URLs) to match the create_with_uploads behavior.
    """
    ...
    file_fields = [
        ("thumbnail", False),
        ("images", True),
        ("file_link", False),  # keep as-is (your router expects this name)
    ]
    ...
    async def execute(
        ...
        try:
            ...
            if thumbnail:
                ...
                key = await _upload_and_return_key(thumbnail.file, fname, f"{dest_prefix}/thumbnail", ct)
                row.thumbnail = key
            ...
            if images:
                for idx, img in enumerate(images):
                    ...
                    key = await _upload_and_return_key(img.file, fname, f"{dest_prefix}/images", ct)
                    if key:
                        new_keys.append(key)
            ...
            if payload.images_clear or new_keys or (row.images is None):
            ...
            if file_link:
                ...
                key = await _upload_and_return_key(file_link.file, fname, f"{dest_prefix}/files", ct)
                row.file_link = key
            ...
            # Build signed urls (best-effort) without changing stored values
            signed_thumb = None
