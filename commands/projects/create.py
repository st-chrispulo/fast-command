...
try:
    ...
    logger = _logging.getLogger("projects_create_with_uploads")
...
class CreateProjectWithUploadsCommand(BaseCommand):
    """
    Creates a Project with optional uploads:
      - thumbnail: UploadFile (single)
      - images: List[UploadFile]
      - attachment: UploadFile -> stored as file_link
    """
    name = "projects/create_with_uploads"
    ...
    async def execute(
        ...
        t0 = time.monotonic()
        logger.info("[projects] start name=%s user_id=%s", getattr(payload, "name", None), user_id)
        gcs = get_gcs()

        project_id = str(uuid4())
        dest_prefix = f"{self.base_folder}/{project_id}"
        ...
        try:
            ...
            db.rollback()
            logger.exception("[projects] DB error")
            raise
        finally:
            db.close()
