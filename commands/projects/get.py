...
def _apply_sort(q, sort_key: str, sort_order: str):
    col_expr = func.lower(Project.name) if sort_key == "name" else getattr(Project, sort_key, Project.created_at)
    return q.order_by(asc(col_expr) if sort_order == "asc" else desc(col_expr))


def _paginate(q, page: int, limit: int) -> Tuple[List[Project], int]:
...
def _serialize_project(row: Project, gcs) -> Dict[str, Any]:
...
class ProjectGetCommand(BaseCommand):
    """
    GET /projects/get
    """
    name = "projects/get"
    schema = ProjectListQuery
    require_auth = True
    method = "GET"
    group = "Project"

    def execute(self, payload: ProjectListQuery, user_id: str):
        if not user_id:
        ...
        with SessionLocal() as session:
            try:
                q = session.query(Project).filter(Project.created_by == user_id)
                ...
                if tags_list:
                    ...
                    q = q.filter(Project.tags.op("&&")(typed_array))
                    # or: q = q.filter(Project.tags.overlap(typed_array))
                ...
                gcs = get_gcs()
                resp["data"] = [_serialize_project(row, gcs) for row in items]
    ...
    def run(self, payload: ProjectListQuery, user_id: str = None):
